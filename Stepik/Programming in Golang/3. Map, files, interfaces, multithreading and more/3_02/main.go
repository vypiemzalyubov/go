...
// }
// Однако база данных с информацией о точной численности населения содержала ошибки, поэтому в cityPopulation в т.ч. была сохранена информация о городах,
// которые входят в другие группы из groupCity.
...
func main() {
	...
	for cityPop := range cityPopulation {
		found := false
		for _, cityGroup := range groupCity[100] {
			if cityPop == cityGroup {
				found = true
				continue
			}
		}
		if !found {
			delete(cityPopulation, cityPop)
		}
	}
