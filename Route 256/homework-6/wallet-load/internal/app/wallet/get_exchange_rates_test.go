...
const (
	wiremockURL = "http://localhost:8070"
	testEnv     = "test"
)
...
func TestGetExchangeRates_Positive(t *testing.T) {
	...
	t.Run("Simplr get exchange rates", func(t *testing.T) {
		...
		if err := wiremockClient.Reset(); err != nil {
			t.Fatalf("Failed to reset wiremock client: %v", err)
		}

		if err := wiremockClient.StubFor(wiremock.Get(wiremock.URLPathEqualTo("/scripts/XML_daily.asp")).
			...
			t.Fatalf("Failed to stub for wiremock: %v", err)
		}
...
func TestGetExchangeRates_Negative(t *testing.T) {
	...
	t.Run("Internal CBR error", func(t *testing.T) {
		...
		if err := wiremockClient.StubFor(wiremock.Get(wiremock.URLPathEqualTo("/daily_json.js")).
			WillReturnResponse(
				wiremock.NewResponse().
